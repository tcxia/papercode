...
class PTuneForLAMA(torch.nn.Module):
    def __init__(self, args, device, template) -> None:
        super().__init__()
        self.args = args
        self.device = device
        ...
        self.embedding = get_embedding_layer(self.args, self.model)
        ...
        self.pesudo_token_id = self.tokenizer.get_vocab()[self.args.pseudo_token]
    ...
    def embed_input(self, queries):
        ...
        queries_for_embedding[(queries == self.pesudo_token_id)] = self.tokenizer.unk_token_id
        raw_embeds = self.embedding(queries_for_embedding)

        if self.args.use_original_template:
            return raw_embeds
        
        bloced_indices = (queries == self.pesudo_token_id).nonzero().reshape((bz, self.spell_length, 2))[:, :, 1]
        replace_embeds = self.prompt_encoder()

        for bidx in range(bz):
            for i in range(self.prompt_encoder.spell_length):
                raw_embeds[bidx, bloced_indices[bidx, i], :] = replace_embeds[i, :]
            return raw_embeds
